...
func UpdatePublisherEnv(media MediaConfig) error {
	...
	*/
	...
	// Transport selection and its optional endpoints. Empty values are written
	// deliberately: the publisher treats an empty URL as "this transport is not
	// available" and falls through, so absent config degrades to today's behaviour.
	fmt.Fprintf(&buf, "ROVERD_VIDEO_TRANSPORT=%s\n", media.Video.Transport)
	fmt.Fprintf(&buf, "ROVERD_VIDEO_RTSP_URL=%s\n", media.Video.RTSPURL)
	fmt.Fprintf(&buf, "ROVERD_VIDEO_WHIP_URL=%s\n", media.Video.WHIPURL)
	...
	fmt.Fprintf(&buf, "ROVERD_AUDIO_CAPTURE_TRANSPORT=%s\n", media.AudioCapture.Transport)
	fmt.Fprintf(&buf, "ROVERD_AUDIO_CAPTURE_RTSP_URL=%s\n", media.AudioCapture.RTSPURL)
	fmt.Fprintf(&buf, "ROVERD_AUDIO_CAPTURE_WHIP_URL=%s\n", media.AudioCapture.WHIPURL)
	...
	fmt.Fprintf(&buf, "ROVERD_AUDIO_PLAYBACK_TRANSPORT=%s\n", media.AudioPlayback.Transport)
	fmt.Fprintf(&buf, "ROVERD_AUDIO_PLAYBACK_RTSP_URL=%s\n", media.AudioPlayback.RTSPURL)
